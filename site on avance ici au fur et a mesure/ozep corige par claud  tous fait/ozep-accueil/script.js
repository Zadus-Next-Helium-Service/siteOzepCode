...
if (window.matchMedia('(max-width: 600px)').matches) {
  ...
  if (whyImageParallax) {
    ...
    function updateWhyImageParallax() {
      ...
      const offset = progress < 0.5 ? (progress - 0.5) * 300 : (progress - 0.5) * 520;
...
// Footer mobile : les colonnes (Navigation, Domaines, Contact) se
// déplient/replient au toucher, au lieu d'être toutes affichées
// empilées en permanence.
if (window.matchMedia('(max-width: 600px)').matches) {
  document.querySelectorAll('.footer-col h5').forEach(function (h5) {
    h5.addEventListener('click', function () {
      h5.parentElement.classList.toggle('is-open');
    });
  });
}

// Blocs "expertise détaillée" (page services) : même calcul direct de
// scroll que pour les autres sections, plus fiable que le mécanisme
// générique sur certains téléphones.
if (window.matchMedia('(max-width: 600px)').matches) {
  const expertiseEls = document.querySelectorAll('.expertise-text, .expertise-image');
  if (expertiseEls.length) {
    let tickingExpertise = false;

    function updateExpertiseReveal() {
      const triggerLine = window.innerHeight * 0.82;
      expertiseEls.forEach(function (el) {
      ...
      tickingExpertise = false;
    }

    window.addEventListener('scroll', function () {
      if (!tickingExpertise) {
        window.requestAnimationFrame(updateExpertiseReveal);
        tickingExpertise = true;
      }
    }, { passive: true });
...
// Bandes de l'accordéon "Nos domaines d'activité" (page services) :
// apparition en rotation 3D, l'une après l'autre en scrollant.
if (window.matchMedia('(max-width: 600px)').matches) {
  const accItems = document.querySelectorAll('.acc-item');
  if (accItems.length) {
    let tickingAccItems = false;

    function updateAccItemsReveal() {
      ...
      tickingAccItems = false;
    }

    window.addEventListener('scroll', function () {
      if (!tickingAccItems) {
        window.requestAnimationFrame(updateAccItemsReveal);
        tickingAccItems = true;
      }
    }, { passive: true });

    updateAccItemsReveal();
  }
}
...
const revealElements = Array.from(
  ...
  return !(isMobile && (
    ...
    el.classList.contains('why-item') ||
    el.classList.contains('expertise-text') ||
    el.classList.contains('expertise-image') ||
    el.classList.contains('realisations-text') ||
    el.classList.contains('realisations-image')
  ));
});
